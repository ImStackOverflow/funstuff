...
class ChessBoard
{
                    ...
                    for (int destRow = 1; destRow <= board_size; destRow++)
                    {
                        ...
                        if (possible[0])
                        {
                            ...
                            if (level == ogLevel )
                            {
                                ...
                                {
                                    writeToAnalysisFile(Character.toString(Utilities.returnChessPieceType(nodeSrc)));
                                    for (int i = 0; i < 4; i++)
                                    {
                                        writeToAnalysisFile(" " + Integer.toString(nextTry[i]));
                                    }
                                    return true;
                                }
                                else continue;
    ...
    public static void performOperations(ChessBoard c, int level)
    {
        ...
        {
            Utilities.errExit(e.getMessage());
        }
