...
int main(void)
{
     ...
     **************************************************************************************************/
    OledInit();
    OledDrawString("Turn the Potentiometer");
    OledUpdate();
    char a[100]; //store message
    int percent;

    for ever {
	if (timer.event == 1) {
	    percent = timer.counter * 100 / 1023; //calculate percent
	    sprintf(a, "Pot value: \n%d \n%d%%", timer.counter, percent);
	    OledDrawString(a);
	    OledUpdate();
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void)
{
    ...
    int average; //average adc value
    average = (ADC1BUF0 + ADC1BUF1 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7) / 8;
    if (timer.counter != average) {//if theres a change
	timer.event = 1; //event
	timer.counter = average;
    }
}